...
- Trigger informer caches drop `metadata.managedFields` to save memory. Those caches are also what v2 trigger expressions resolve `resource` against, so `TEST_TRIGGER_KEEP_MANAGED_FIELDS=true` keeps them for deployments whose expressions read managed fields.

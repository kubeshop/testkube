...
- `pkg/testresults/` parses test reports, decides the per-test-case verdict for a step's `testCases` block, and works out which test cases a run should be narrowed to. `junit.go` streams a JUnit report token-by-token (a ten-thousand-case file is several megabytes and the init process holds the test tool's memory alongside its own); `selector.go` matches test cases by glob over `TestCase.ID()`; `verdict.go` applies mute, then the pass requirement, then `enforce`; `selection.go` filters a previous report down to what is worth re-running and projects it into the shape the user's tool wants.
- **Selection runs before the step's command, the verdict after it.** That ordering is the whole narrowing-retry mechanism, in `cmd/testworkflow-init/commands/run.go`: for a tool that overwrites its report, the same file holds the previous attempt's results when the selection reads it and this attempt's when the verdict reads it, so nothing has to track attempt numbers. Do not move either side of it.
- `testCases.selected` is **always a list, empty when nothing was selected.** That is what lets one `args: ['{{ testCases.selected }}']` line serve both a full run and a narrowed one - an empty list expands to no arguments, so the user writes no conditional. Preserve the property.
- Muted test cases are excluded from a selection by default (`select.includeMuted` opts back in). Without that a narrowing retry never converges: a muted failure fails every attempt, gets re-selected every attempt, and burns the whole retry budget on a test whose outcome was already decided not to matter.

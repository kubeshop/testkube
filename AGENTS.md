...
- `common.ControlPlaneAPIURI` (`common/flags.go`) chooses the host a lookup talks to, and which URI is right depends on what the command is for. A command that *modifies an existing context* (`set context`) must use this helper: with no explicit flag `ProcessMasterFlags` composes the SaaS host, which would both look the name up on the wrong Control Plane and persist that host over the configured one.
- A command that *targets a Control Plane named by its flags* (`login`, `pro init`, `docker init`, and the connection itself in `pro connect`) deliberately keeps `opts.Master.URIs.Api`. Its token is issued by that same host, so redirecting only the lookup would present a token from one Control Plane to another. In `pro init` and `docker init` the stored token is reachable only after `IsUserLoggedIn`, whose first check already requires the composed and stored URIs to be equal, so the helper would be a no-op there anyway.

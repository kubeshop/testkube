...
- `pkg/testworkflows/localrunner/` owns offline YAML/schema validation, Kind/k3d context safety, the supported-feature gate, direct Kubernetes-worker execution, source and artifact relay handling, safe local artifact extraction, pause control through API-server port-forwarding, and exact run-label cleanup. `pkg/testworkflows/localartifacts/` defines the private relay protocol and its portable-path validation. Source-relay paths and local-artifact bearer tokens are sensitive and must never be logged.
- Local artifact export is opt-in: a workflow `artifacts` block requires `testkube local run --artifacts-dir <directory>`, and that flag requires at least one artifact block. The default total local artifact-export bound is 100 MiB and `--max-artifact-bytes` changes that bound. A successful export belongs only below `<artifacts-dir>/<run-id>/`, with per-step `steps/<TK_REF>/` subdirectories; never use a workflow-controlled archive path to escape that root or overwrite another run.
- The artifact token flows through the exact-run Kubernetes Secret/environment only. Do not put it in `TK_CFG`, annotations, CLI arguments, error strings, or logs. The artifact stage uses the exact ready receiver Pod address rather than a selector Service, so a different Pod cannot join an observable Service selector and receive artifact traffic. The run-owned source relay, artifact relay, and relay Secret all need the ordinary exact local-run labels.
- Every local run uses `testkube.io/local=true` and an exact `testkube.io/local-run-id` label. Cleanup must select that exact run and must never delete the `testkube-local` namespace or pre-existing user Secrets/ConfigMaps. New generated runtime resources need the label propagation seam in `pkg/testworkflows/testworkflowprocessor.Bundle.AddLabels`. `local clean` removes Kubernetes resources only; it deliberately leaves the downloaded host artifact directory intact.
- Keep the support boundary explicit: local mode has no Testkube persistence, history, analytics, webhooks, Testkube artifact storage, resource metrics, API token, or Control Plane configuration. An artifact block in local mode is a direct workflow-to-local-filesystem export through the run-owned relay, never a Testkube API or object-store upload. It does still need normal Kubernetes credentials and, today, registry-resolvable runtime image metadata.

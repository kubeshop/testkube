...
func getSlavesCount(count testkube.Variable) (int, error) {
	if count.Value == "" {
		output.PrintLog(fmt.Sprintf("Slaves count not provided in the SLAVES_COUNT env variable. Creating pod with default slaves %v", DEFAULT_SLAVES_COUNT))
		return DEFAULT_SLAVES_COUNT, nil
	}

...
func (r *JMeterRunner) Run(ctx context.Context, execution testkube.Execution) (result testkube.ExecutionResult, err error) {
	...
	// JMETER_PROPERTIES will passed with -G
	params := make([]string, 0, len(envManager.Variables))
	for _, value := range envManager.Variables {
		if value.Name == jmeter_env.GLOBAL_JMETER_PROPERTIES {
			params = append(params, jmeter_env.FormatJmeterProperties(value.Value)...)
			// continue
		}
		if value.Name == jmeter_env.MASTER_OVERRIDE_JVM_ARGS || value.Name == jmeter_env.MASTER_ADDITIONAL_JVM_ARGS{

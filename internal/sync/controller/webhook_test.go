...
func TestWebhookSyncReconcilerSkipsWhenNoGitOpsSyncAnnotationIsSet(t *testing.T) {
	store := &fakeStore{}
	...
	if store.UpdateCalls != 0 {
		t.Errorf("TestWebhookSyncReconcilerSkipsWhenNoGitOpsSyncAnnotationIsSet: expected 0 update calls, got %d", store.UpdateCalls)
	}

	if store.Deleted != "" {
		t.Errorf("TestWebhookSyncReconcilerSkipsWhenNoGitOpsSyncAnnotationIsSet: expected no delete call, got %q", store.Deleted)
	}
}

...
// ServiceInstance represents a single service instance to be started.
// Contains all configuration needed to deploy and monitor the service.
...
// Used to track IP assignment and description for service discovery.
...
// AsMap converts ServiceInfo to a map for JSON output.
...
type ServiceExecutionResult struct {
	Started bool  // Container started running
	Ready   bool  // Container passed readiness check (or N/A)
	Failed  bool  // Execution finished with failure status
	Error   error // Any error during monitoring
}

const (
	// UnlimitedParallelism indicates no limit on parallel service instances.
	UnlimitedParallelism = math.MaxInt64

	// serviceFailureCheckTimeout is how long to wait for Kubernetes to report
	// a container exit after startup. The kubelet syncs pod status every 10s
	// by default, so we need to wait longer than that.
	serviceFailureCheckTimeout = 15 * time.Second
)

// ServicesDependencies contains external dependencies for services execution.
// Injected from the command handler to enable testing.
...
// Coordinates parsing, preparation, execution, and result reporting.
...
// NewServicesExecutor creates a new services executor with injected dependencies.
...
// NewServicesCmd creates the cobra command for services.
...
func (e *ServicesExecutor) Execute(args []string) error {
	...
	// Parse services from arguments
	services, err := e.parseServices(args)
	if err != nil {
		return err
	}

	// Apply defaults and initialize details
	...
	// Notify about queued instances
	e.notifyQueuedInstances(instances)

	// Start transfer server if needed
...
// parseServices parses service specifications from command arguments.
// Supports two input formats:
//   - Base64 encoded JSON (--base64 flag): Used by processor to prevent premature expression resolution
//   - Raw JSON key=value pairs (legacy): Kept for backward compatibility
...
// initializeServiceDefaults applies default values and initializes state for each service.
...
// Returns instances, namespaces, state map, and parameters for each service.
...
// Run executes the service and returns true if successful, false otherwise.

...
1. **One action group per container** - Each container runs one group index
2. **Check state consistency** - Verify state file exists before running subsequent groups
3. **Handle failures gracefully** - Use proper exit codes
4. **Enable debug mode for troubleshooting** - Set DEBUG=1 for verbose output

...
// RunInit runs the full init process logic (extracted from main.go)
func RunInit(groupIndex int) int {
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		if err := os.MkdirAll(dir, 0777); err != nil {
			...
			output.ExitErrorf(constants.CodeInternal, "failed to create directory %s: %v", dir, err)
		}
	...
	for i, action := range actions {
		...
		case lite.ActionTypeExecute:
			...
			for {
				...
				config := newMetricsRecorderConfig(step.Ref, action.Execute.Toolkit, state.ContainerResources)
	...
	_ = orchestration.Executions.Kill()
	if orchestration.Executions.IsAborted() {
		return int(constants.CodeAborted)
	} else {
		return 0
	}
}

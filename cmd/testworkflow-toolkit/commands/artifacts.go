	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, paths []string) {
			...
			postProcessors = append(postProcessors,
				artifacts.NewCustomMetricsPostProcessor(filesystem.NewOSFileSystem(), client, cfg.Execution.EnvironmentId, cfg.Execution.Id, cfg.Workflow.Name, config.Ref(), walker.Root(), cfg.Resource.FsPrefix),
				artifacts.NewK6SummaryPostProcessor(filesystem.NewOSFileSystem(), client, cfg.Execution.EnvironmentId, cfg.Execution.Id, cfg.Workflow.Name, config.Ref(), walker.Root(), cfg.Resource.FsPrefix),
			)

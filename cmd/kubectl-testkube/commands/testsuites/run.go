...
func NewRunTestSuiteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			options.ExecutionVariables, err = common.CreateVariables(cmd, !info.SecretConfig().AutoCreate)
	...
	cmd.Flags().StringSliceVarP(&selectors, "label", "l", nil, "label expression to select test suites to run using key value pair: --label key1=value1")

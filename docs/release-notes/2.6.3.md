...
* **Webhook Event Race Condition with Multiple Listeners and Become Events** ([#7056](https://github.com/kubeshop/testkube/pull/7056))
  ...
  **Root Cause:**
  ...
  - When multiple webhooks matched different event types (e.g., webhook A listens to `start-*`, webhook B listens to `end-*`), the goroutines racing to filter event types would overwrite each other's `event.EventType` values
  ...
  **Technical Details:**
  - Shallow copy of the event is safe because only the `EventTypes` field is modified
  - Event type filtering now properly creates value copies to avoid pointer aliasing
...
* **Event Idempotency Using TTL Cache** ([#7052](https://github.com/kubeshop/testkube/pull/7052))
  ...
  **Root Cause:**
  - Events could be redelivered or replayed due to various reasons (network issues, Kafka retries, system restarts)
  ...
  **Technical Details:**
  ...
  - Comprehensive test coverage including:
    ...
    - Event processing after TTL expiration
...
### Bug Fixes
* 35ce4ee54: fix: webhook event race condition with multiple listeners and become events (#7056) (@Copilot, @vsukhin)
* 01ae5920f: fix: add event idempotency using TTL cache on event.Id with configurable TTL and capacity (#7052) (@Copilot, @vsukhin)

### Other Changes
* 47bdb1441: fix: go fmt (@vsukhin)

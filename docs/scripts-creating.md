...
- A simple file with test content. For postman collections, we're exporting collection as JSON file.  For curl executor, we're passing JSON file with configured curl command.
- `Git` - we can pass `repository`, `path` and `branch` where our test is. This is used in Cypress executor - as Cypress tests are more like npm-based projects which can have a lot of files. Sparse checkouts are handled that are fast even in the case of huge mono-repos.
...
You can also check scripts with standard `kubectl` command which will list the Scripts Custom Resource.

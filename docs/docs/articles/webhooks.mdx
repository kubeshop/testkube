...
Webhook payload can contain Event-specific variables - they will be replaced with actual data when events occurs. In the above examples, only the event `Id` is being sent. 
However, any of [supported Event Variables](#supported-event-variables) can be used.

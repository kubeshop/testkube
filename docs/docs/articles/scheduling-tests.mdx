...
Testkube's schedule data format is the same that is used to define Kubernetes Cron jobs (check [Wikipedia Cron](https://en.wikipedia.org/wiki/Cron) format for details).

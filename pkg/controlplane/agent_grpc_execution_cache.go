...
func (s *Server) lookupCacheEntry(ctx context.Context, scope cacheScope, key string, restoreKeys []string) (*cloud.GetExecutionCachePresignedResponse, error) {
	...
	// Listed under each restore key rather than across the whole scope.
	//
	// The candidate limit applies to each listing, and a store returns that page in
	// lexical order - so listing the scope and filtering afterwards spent the whole
	// budget on keys that could never match, and in a scope with many entries the
	// matches sat past the end of the page. That surfaced as a miss, or as an older
	// entry winning over a newer one that was never listed. Entries are immutable and
	// only leave on expiry, so a busy scope reaches that size as a matter of course.
	//
	// The cost is one listing per restore key instead of one in total, bounded by the
	// ten keys a step may declare.
	entries := make([]executioncache.Entry, 0, MaxCacheRestoreCandidates)
	seen := make(map[string]struct{})
	for _, prefix := range prefixes {
		listed, err := s.listCacheEntries(ctx, prefix, MaxCacheRestoreCandidates)
		if err != nil {
			return nil, err
		}
		for _, entry := range listed {
			// Restore keys may nest - "npm-" covers everything "npm-v3-" does - and
			// MatchRestore reads a repeated entry as a genuine candidate each time.

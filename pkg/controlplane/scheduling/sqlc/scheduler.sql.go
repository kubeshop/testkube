...
WHERE id = $3::text RETURNING id, name, namespace, number, test_workflow_execution_name, group_id, runner_id, runner_target, runner_original_target, disable_webhooks, tags, running_context, config_params, silent_mode, scheduled_at, assigned_at, status_at, created_at, updated_at, organization_id, environment_id, runtime
`
...
SELECT
    e.id, e.name, e.namespace, e.number, e.test_workflow_execution_name, e.group_id, e.runner_id, e.runner_target, e.runner_original_target, e.disable_webhooks, e.tags, e.running_context, e.config_params, e.silent_mode, e.scheduled_at, e.assigned_at, e.status_at, e.created_at, e.updated_at, e.organization_id, e.environment_id, e.runtime,

...
func (s *Service) releaseDynamicInformerForTrigger(t *testtriggersv1.TestTrigger, key statusKey) {
	if t.Spec.ResourceRef == nil || s.dynamicManager == nil || isBuiltinResource(t.Spec.ResourceRef.Kind) {
		return
	}
	gvr, err := resolveGVR(s.dynamicManager.mapper, t.Spec.ResourceRef.Group, t.Spec.ResourceRef.Version, t.Spec.ResourceRef.Kind)
	if err != nil {
...
func (s *Service) updateWorkflowTrigger(target *workflowtriggersv1.WorkflowTrigger) {
	...
	if s.triggerStatus[key] != nil {
		...
		if gvrChanged && old.ResourceKind != "" && !isBuiltinResource(old.ResourceKind) && s.dynamicManager != nil {
			if gvr, err := resolveGVR(s.dynamicManager.mapper, old.ResourceGroup, old.ResourceVersion, old.ResourceKind); err == nil {
				s.dynamicManager.releaseInformer(gvr, string(key))
			}
		}
...
func (s *Service) removeWorkflowTrigger(target *workflowtriggersv1.WorkflowTrigger) {
	key := newStatusKey(triggerSourceV2, target.Namespace, target.Name)
	delete(s.triggerStatus, key)
	if target.Spec.Watch == nil || s.dynamicManager == nil || isBuiltinResource(target.Spec.Watch.Resource.Kind) {
		return
	}
	if gvr, err := resolveGVR(s.dynamicManager.mapper, target.Spec.Watch.Resource.Group, target.Spec.Watch.Resource.Version, target.Spec.Watch.Resource.Kind); err == nil {
		s.dynamicManager.releaseInformer(gvr, string(key))
	}
}

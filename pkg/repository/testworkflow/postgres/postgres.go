...
func (r *PostgresRepository) GetLatestExecutionsByWorkflow(ctx context.Context, filter testworkflow.Filter) ([]testkube.TestWorkflowExecution, error) {
	...
	rows, err := tx.Query(ctx, getLatestTestWorkflowExecutionsByWorkflowQuery,
	...
	var items []sqlc.GetTestWorkflowExecutionsRow
	for rows.Next() {
		var i sqlc.GetTestWorkflowExecutionsRow
		if err := rows.Scan(
			&i.ID,
			&i.GroupID,
			&i.RunnerID,
	...
	result := make([]testkube.TestWorkflowExecution, len(items))
...
const getLatestTestWorkflowExecutionsByWorkflowQuery = `-- name: GetLatestTestWorkflowExecutionsByWorkflow :many
SELECT * FROM (

...
// stepIdPattern allows lowercase alphanumeric and underscores, must start with a letter.
...
// characters and underscores, and starts with a letter.
func ValidateStepId(id string) error {
	if !stepIdPattern.MatchString(id) {
		return fmt.Errorf("step id %q must contain only lowercase alphanumeric characters and underscores, and start with a letter", id)
	}
	return nil
}

// DeriveStepId converts a step name to a valid identifier by lowercasing
// and replacing all non-alphanumeric characters with underscores.
// Returns empty string if the name cannot produce a valid ID.

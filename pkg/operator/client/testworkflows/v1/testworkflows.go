...
//go:generate go tool mockgen -destination=./mock_testworkflows.go -package=v1 "github.com/kubeshop/testkube/pkg/operator/client/testworkflows/v1" Interface
...
func (s TestWorkflowsClient) Apply(workflow *testworkflowsv1.TestWorkflow) error {
	obj, err := runtime.DefaultUnstructuredConverter.ToUnstructured(workflow)
	if err != nil {
		return err
	}

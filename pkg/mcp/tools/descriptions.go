...
'failed', 'skipped', 'aborted', 'timeout', 'paused'`
	...
	FetchExecutionLogsDescription     = "Retrieves logs from a test workflow execution. Default (no params, or step-only): returns the last 100 lines. When using grep, searches the full log (server caps at 100 matching lines). Always work in chunks of 100 lines when paging — never request unbounded ranges. Parameters: tail (last N lines, e.g. tail=100), startLine/endLine (1-based range, e.g. startLine=1 endLine=100), grep (substring filter), step (filter to a specific step reference). Parameters can be combined. For parallel workflows, set workerRef (step reference) and optionally workerIndex (0-based, default 0) to fetch a specific worker's logs instead of the main execution log. Discover available workers and their refs via get_execution_info."

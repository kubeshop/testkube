...
'failed', 'skipped', 'aborted', 'timeout', 'paused'`
	...
	RunWorkflowDescription           = "Run a TestWorkflow with optional configuration parameters and target specification. If the workflow requires config parameters, use the get_workflow_definition tool first to examine the spec.config section to see what parameters are available. IMPORTANT: All config parameter values must be passed as strings (e.g., {\"vus\": \"40\", \"duration\": \"20s\"}) to avoid API errors. The target parameter supports multiple formats: 1) {\"name\": \"agent-name\"} to target a specific runner by name, 2) {\"labels\": {\"env\": \"prod\", \"type\": \"runner\"}} to target runners by labels, 3) Standard ExecutionTarget format with match/not/replicate fields."

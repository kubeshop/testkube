...
'failed', 'skipped', 'aborted', 'timeout', 'paused'`
	...
	BuildDashboardUrlDescription  = `Build dashboard URLs for Testkube workflows and executions. Supports deep linking to a specific step within an execution's log view using stepRef (obtained from execution info signatures) and executionTab (defaults to 'log-output' when stepRef is provided). URL pattern: /organization/{orgId}/environment/{envId}/dashboard/test-workflows/{name}/execution/{execId}/{executionTab}?ref={stepRef}`
